...
class Pedido(models.Model):
    ...
    * Transferir pedidos entre mesas. (menos prioritario)
    ...
    reserva = models.ForeignKey('clientes.Reserva', limit_choices_to={'estado__reserva_estado': "VIG"},
    ...
    monto_entrega_reserva = models.DecimalField(max_digits=18, decimal_places=0, default=0,
    ...
    # Filtrar los Empleados que tienen el cargo de Mozo y Barman
    mozo_pedido = models.ForeignKey('personal.Empleado', limit_choices_to=Q(cargo__cargo__exact="MO") |
                                                                          Q(cargo__cargo__exact="BM"),
                                    verbose_name='Atendido por?',
                                    help_text='Seleccione el personal que atendio al Cliente.')
    ...
    fecha_pedido = models.DateTimeField(auto_now_add=True, verbose_name='Fecha/Hora del Pedido',  # default=timezone.now()
    ...
    # 2) Al guardar el Pedido se debe cambiar el estado de la Reserva a UTILIZADA en caso de que se haya utilizado una.
    # 3) Al guardar el Pedido se debe descontar los productos solicitados del Stock.
    ...
    def __unicode__(self):
        return "Nro. Ped: %s - Fec. Ped: %s" % (self.id, datetime.datetime.strptime((self.fecha_pedido), '%Y-%m-%d').strftime('%d/%m/%Y'))
...
class PedidoDetalle(models.Model):
    ...
    * Imprimir comanda.
    ...
    # 2) Calcular automaticamente el campo "total_producto_pedido" como la multiplicacion de "precio_producto_pedido * cantidad_producto_pedido"
...
class Venta(models.Model):
    ...
    * Listar las ventas mensuales, ventas al contado. Ranking de ventas por cliente, por productos, por marcas.
    ...
    numero_pedido = models.ForeignKey('Pedido', limit_choices_to={'estado_pedido__pedido_estado': "VIG"},  # default=1,
