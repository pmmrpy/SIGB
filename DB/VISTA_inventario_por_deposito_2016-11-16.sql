...
CREATE OR REPLACE VIEW inventario_por_deposito AS  (
 select id, producto,
 sum(cant_exist_dce) as cant_exist_dce,
 sum(cant_exist_dbp) as cant_exist_dbp,
 sum(cant_exist_dba) as cant_exist_dba,
 sum(cant_exist_dco) as cant_exist_dco,
 sum(cant_exist_dbi) as cant_exist_dbi,
 sum(cant_exist_dce + cant_exist_dbp + cant_exist_dba + cant_exist_dco + cant_exist_dbi) as cant_existente
           ...
           FROM ( SELECT producto.id,
             ...
             FULL JOIN ( SELECT producto.id,
                   ...
                   FROM stock_movimientostock stock
                     ...
                     JOIN bar_deposito deposito ON deposito.id = stock.ubicacion_origen_id
                  WHERE deposito.deposito::text = 'DBI'::text
                  GROUP BY producto.id, producto.producto, deposito.deposito) mov_saliente_dbi ON mov_entrante_dbi.id = mov_saliente_dbi.id) dbi ON dce.id = dbi.id) T1
--   from T1
GROUP BY id, producto ORDER BY producto
)
